#!/usr/bin/env python
"""
train_learnable_source.py — NN potential + learnable source position

Model 6: Simultaneously learn the potential (NN) and the source position
(mu_x, mu_y), with death rate and sigma_x, sigma_y fixed.

Source distribution (reparameterization trick):
    x_i(0) = mu_x + eps_i^x * sigma_x     (mu_x learnable)
    y_i(0) = mu_y + eps_i^y * sigma_y     (mu_y learnable)

where eps ~ N(0, 1) is sampled once and reused every epoch.
Gradients flow from loss -> z_init -> (mu_x, mu_y) via jnp.where in the
wake-up mechanism of the SDE solver.

Usage:
    python train_learnable_source.py > log.txt 2>&1
"""
import matplotlib
matplotlib.use('Agg')
...
# Ground-truth source (for target simulation)
SOURCES = [
...
# Learnable source: initial guesses (true = 0.0, -1.0)
INIT_MU_X = -0.5
INIT_MU_Y = -1.5
...
class PotentialWithConfinement(eqx.Module):
    """NN potential + fixed quartic confinement.
...
# ==========================================================================
# 3. Model with learnable source position
# ==========================================================================

class SourceDeathModelWithSource(eqx.Module):
    """SourceDeathModel + learnable source position (mu_x, mu_y).

    mu_x, mu_y are JAX arrays (traced/learnable).
    sigma_x, sigma_y are static (fixed).
    """
    potential: eqx.Module
    death_rate: eqx.Module
    mu_x: jax.Array              # learnable
    mu_y: jax.Array              # learnable
...
def make_z_init(model, eps):
    ...
    Args:
        model: SourceDeathModelWithSource (has mu_x, mu_y, sigma_x, sigma_y)
        eps: (N, 2) standard normal samples (fixed throughout training)

    Returns:
        z_init: (N, 2) differentiable w.r.t. model.mu_x and model.mu_y
    """
...
# ==========================================================================
# 6. Objective (z_init computed from learnable mu_x)
# ==========================================================================

def make_objective(n_steps, dt, sigma, lam_mass):
    """Build loss function. z_init is computed inside from model.mu_x + eps*sigma."""

    def objective(model, eps, wake_schedule,
                  tgt_z, tgt_S, tgt_mass, key):
        # Reparameterization trick: z_init depends on model.mu_x
...
# ==========================================================================
# 8. Generate ground-truth snapshot
# ==========================================================================

print("=" * 65)
print("Model 6: NN potential + learnable source position (mu_x, mu_y)")
print("=" * 65)
...
print(f"  Init mu_y = {INIT_MU_Y} (true = {TRUE_MU_Y})")
print(f"  Learnable: Phi_nn weights + mu_x + mu_y")
print(f"  Fixed: sigma_x={TRUE_SIGMA_X}, "
      f"sigma_y={TRUE_SIGMA_Y}, death rate, quartic conf")
...
# ==========================================================================
# 9. Build training particle queue (reparameterized)
# ==========================================================================

# Pre-sample eps ~ N(0,1) — fixed throughout training
key, eps_key, wake_key = jax.random.split(key, 3)
eps = jax.random.normal(eps_key, (N_PARTICLES, 2))

# Wake schedule: uniform birth times (same structure as target)
...
print(f"  Training queue: {N_PARTICLES} particles, eps pre-sampled\n")
...
model = SourceDeathModelWithSource(
    potential=potential,
    death_rate=fixed_death,
    mu_x=jnp.array(INIT_MU_X),
    mu_y=jnp.array(INIT_MU_Y),
...
print(f"SourceDeathModelWithSource created:")
print(f"  Phi_nn: {n_params_phi} params")
print(f"  mu_x: 1 param (init={INIT_MU_X}, true={TRUE_MU_X})")
print(f"  mu_y: 1 param (init={INIT_MU_Y}, true={TRUE_MU_Y})")
...
N_EPOCHS = 5000
PRINT_EVERY = 40
PATIENCE = 99999  # disable early stopping to see full convergence plot

LR_NN = 3e-3
LR_SOURCE = 2e-2

# Separate optimizers: NN params get AdamW(3e-3), source params get Adam(1e-2)
...
print(f"  Optimizer: NN=AdamW(lr={LR_NN}), source=Adam(lr={LR_SOURCE})")
...
# Save checkpoint for resuming: last model (not best), opt_state, history, epoch
...
# --- Figure 1: Training curves + source convergence ---
...
# Simulate learned model with LEARNED source position
