#!/usr/bin/env python
"""
train_death_wall_16_32_32_16.py — Model 07: Potential-only training (plain x^4)

Ground truth: H = -(y-a)x^2 + exp(b)x^4 - cy + e*x  (no y^4)
Learned: PotentialNN(16,32,32,16) only (no confinement term, no x^4 wrapper)
Source and death rate: FIXED at ground truth values.

Usage:
    python train_death_wall_16_32_32_16.py > log.txt 2>&1
"""
import matplotlib
matplotlib.use('Agg')
...
SCRIPT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nn_16_32_32_16")
os.makedirs(SCRIPT_DIR, exist_ok=True)
SUFFIX = "07_nn16x32x32x16_N3000_Ntgt6000_t3_ep2000"
...
print("  NN architecture: 2 -> 16 -> 32 -> 32 -> 16 -> 1")
print("=" * 65)
print(f"  Ground truth: H = -(y-a)x^2 + exp(b)x^4 - cy + e*x")
print(f"  Learned: PotentialNN(16,32,32,16) (no confinement term)")
...
print(f"  Potential: PotentialNN(2->16->32->32->16->1, c_conf=0.0) — {n_params} params")
...
fig.suptitle(f'Model 07: plain $\\exp(b)x^4$ | NN(16,32,32,16) | death wall y_d={DEATH_PARAMS_EXP["y_d"]}',
             fontsize=13, y=1.01)
