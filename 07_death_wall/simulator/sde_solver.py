...
def simulate_open_system(model, z_init, wake_schedule, sigma, n_steps, dt, key,
    ...
    Args:
        ...
        dt: step size
        key: PRNG key
        z_clamp: optional (d, 2) array [[x_min, x_max], [y_min, y_max]]
                 for position clamping. Prevents numerical instability
                 when potentials lack full confinement (e.g. no y^4).
    ...
    def scan_body(carry, inputs):
        ...
        # 4. Position clamp (prevents Euler divergence for dead particles)
        if use_clamp:
            new_z = jnp.clip(new_z, z_clamp[:, 0], z_clamp[:, 1])

        # 5. Death: dS = -gamma(z) dt
