#!/usr/bin/env python
"""
sweep_nparticles_t10.py — Particle count sweep at T=10 (near steady-state)

Same design as v3 but:
  - T_FINAL=10.0 (mass curve nearly flat, dM/dt~7.5 vs 54 at t=6)
  - N_EPOCHS=2000, PATIENCE=800

Sweep: N_learned = [200, 400, 600, 1000, 1500, 2000, 3000, 4000, 6000]

Usage:
    python sweep_nparticles_t10_ep2000/sweep_nparticles_t10.py > sweep_nparticles_t10_ep2000/log.txt 2>&1
"""
import matplotlib
matplotlib.use('Agg')
...
def find_critical_points_2d(phi_fn, x_range=(-4, 4), y_range=(-2, 3.5),
                            grid_n=200):
    """Find 2D minima (one per side of x=0) and saddle point between them.

    Uses vectorized JAX evaluation on a grid, then scipy for refinement.
    """
    ...
    # Find grid minimum on each side of x=0
    mid = grid_n // 2
    left_grid = Zg[:, :mid]
    right_grid = Zg[:, mid:]
    ...
    # Find saddle: vectorized check for local max in x, local min in y
    # A saddle between two x-wells: max in x-direction, min in y-direction
    max_x = (Zg[1:-1, 1:-1] > Zg[1:-1, :-2]) & (Zg[1:-1, 1:-1] > Zg[1:-1, 2:])
    min_y = (Zg[1:-1, 1:-1] < Zg[:-2, 1:-1]) & (Zg[1:-1, 1:-1] < Zg[2:, 1:-1])
    saddle_mask = max_x & min_y

    # Restrict to region between the two minima in x
    x_lo = min(xL, xR) + 0.2
    x_hi = max(xL, xR) - 0.2
...
if __name__ == '__main__':
    ...
    print(f"  True CP: left=({true_cp['x_min_left']:.3f},{true_cp['y_min_left']:.3f}), "
