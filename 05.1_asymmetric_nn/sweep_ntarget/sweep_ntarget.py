#!/usr/bin/env python
"""
sweep_ntarget.py — Target particle count sweep

Reverse of sweep_nparticles_v3: fixes N_learned=1000, varies N_target to
determine how much training data (real cell data) is needed for accurate
learning.

Experimental design:
  - Target: VARYING N_target with DIFFERENT seed per N_target
  - Learned: FIXED seed=2, FIXED N_learned=1000
  - sigma=1.5 for both, fixed

Sweep: N_target = [2000, 4000, 6000, 8000, 10000, 12000]
       seeds   = [1,    2,    3,    4,    5,     6]
epochs=800, patience=300

Includes distribution analysis:
  - Analysis batch: N_analysis=2000, seed=99
  - Compares: mass curves, bifurcation, marginals, decay curves,
    trajectories, zone fractions

Usage:
    python sweep_ntarget/sweep_ntarget.py > sweep_ntarget/log.txt 2>&1
"""
import matplotlib
matplotlib.use('Agg')
...
N_LEARNED = 1000               # fixed learned particle count
SEED_LEARNED = 99
N_TARGET_LIST  = [2000, 4000, 6000, 8000, 10000, 12000]
TARGET_SEEDS   = [1,    2,    3,    4,    5,     6]

# Analysis
N_ANALYSIS = 2000
SEED_ANALYSIS = 777
N_TRAJ_PLOT = 30
SELECT_N = [2000, 4000, 6000, 8000, 10000, 12000]  # all N_target values
...
def run_single(n_target, tgt_z, tgt_S, tgt_mass, out_dir):
    """
    Train a model with N_learned=N_LEARNED against N_target training data.
    Returns (model, result_dict).
    """
    os.makedirs(out_dir, exist_ok=True)
    print(f"\n  N_target={n_target}")

    # --- Learned particle queue (seed=SEED_LEARNED, N=N_LEARNED) ---
    ...
    # --- Create model (same NN init for all runs) ---
...
if __name__ == '__main__':
    print("=" * 65)
    print("Target particle count sweep")
    print("=" * 65)
    ...
    # ==================================================================
    # True critical points (reference)
    # ==================================================================
    x_line = np.linspace(-4, 4, 500)
    ref_true = float(analytical_potential(0.0, 0.0, TARGET_PARAMS))
    true_profiles = {}
    ...
    print(f"\n  True CP: barrier={true_cp['barrier']:.3f}, "
          f"asym={true_cp['well_asymmetry']:.3f}")

    # ==================================================================
    # Prepare analysis batch (seed=99, N=2000) — shared across all runs
    # ==================================================================
    ...
    for n_target, seed_tgt in zip(N_TARGET_LIST, TARGET_SEEDS):
        ...
        if n_target in SELECT_N:
            lrn_tau, lrn_mean, lrn_lo, lrn_hi, lrn_med = \
                compute_decay_curves(ana_lrn_S, ana_wake, DT)
            stats['decay_tau'] = lrn_tau
            stats['decay_mean'] = lrn_mean
            stats['decay_lo'] = lrn_lo
            stats['decay_hi'] = lrn_hi
            stats['decay_med'] = lrn_med
            stats['all_z'] = np.array(ana_lrn_z)
            stats['all_S'] = np.array(ana_lrn_S)
    ...
    # ==================================================================
    # SUMMARY PLOT 2: Critical points vs N_target
    # ==================================================================
    ...
    # ==================================================================
    # ANALYSIS PLOT 1: Mass curves (selected N_target)
    # ==================================================================
    n_sel_plots = len(SELECT_N)
    n_cols = 3
    n_rows = (n_sel_plots + n_cols - 1) // n_cols
    ...
    for i, n_sel in enumerate(SELECT_N):
        ...
        ax.plot(time_axis, all_analysis[n_sel]['mass_curve'], 'r--',
                linewidth=2, label=f'Learned (Ntgt={n_sel})', alpha=0.8)
        ax.set_xlabel('Time')
        ax.set_ylabel('$M(t)$')
        ax.set_title(f'$N_{{target}}$ = {n_sel}')
        ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
    for i in range(n_sel_plots, len(axes_flat)):
        axes_flat[i].set_visible(False)
    plt.suptitle('Total mass $M(t)$: target vs learned', fontsize=14, y=1.02)
    plt.tight_layout()
    ...
    # ==================================================================
    # ANALYSIS PLOT 3: Weighted marginals for selected N_target
    # ==================================================================
    fig, axes = plt.subplots(2, n_cols, figsize=(18, 10))
    for col, n_sel in enumerate(SELECT_N[:n_cols]):
        axes[0, col].plot(x_bins, x_hist_tgt, 'b-', lw=2, label='Target')
        axes[0, col].plot(x_bins, all_analysis[n_sel]['x_hist'], 'r--', lw=2,
                          label=f'Learned (Ntgt={n_sel})')
        axes[0, col].set_xlabel('x')
        axes[0, col].set_ylabel('Weighted density')
        axes[0, col].set_title(f'x-marginal ($N_{{tgt}}$={n_sel})')
        axes[0, col].legend(fontsize=9); axes[0, col].grid(True, alpha=0.3)

        axes[1, col].plot(y_bins, y_hist_tgt, 'b-', lw=2, label='Target')
        axes[1, col].plot(y_bins, all_analysis[n_sel]['y_hist'], 'r--', lw=2,
                          label=f'Learned (Ntgt={n_sel})')
        axes[1, col].set_xlabel('y')
        axes[1, col].set_ylabel('Weighted density')
        axes[1, col].set_title(f'y-marginal ($N_{{tgt}}$={n_sel})')
        axes[1, col].legend(fontsize=9); axes[1, col].grid(True, alpha=0.3)
    plt.suptitle('Weighted spatial marginals at t=6.0 (first 3)',
                 fontsize=14, y=1.01)
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, "analysis_marginals_1.png"),
                dpi=150, bbox_inches='tight')
    plt.close()
    print("Saved analysis_marginals_1.png")

    if len(SELECT_N) > n_cols:
        fig, axes = plt.subplots(2, n_cols, figsize=(18, 10))
        remaining = SELECT_N[n_cols:]
        for col, n_sel in enumerate(remaining):
            axes[0, col].plot(x_bins, x_hist_tgt, 'b-', lw=2, label='Target')
            axes[0, col].plot(x_bins, all_analysis[n_sel]['x_hist'], 'r--',
                              lw=2, label=f'Learned (Ntgt={n_sel})')
            axes[0, col].set_xlabel('x')
            axes[0, col].set_ylabel('Weighted density')
            axes[0, col].set_title(f'x-marginal ($N_{{tgt}}$={n_sel})')
            ...
            axes[1, col].plot(y_bins, y_hist_tgt, 'b-', lw=2, label='Target')
            axes[1, col].plot(y_bins, all_analysis[n_sel]['y_hist'], 'r--',
                              lw=2, label=f'Learned (Ntgt={n_sel})')
            axes[1, col].set_xlabel('y')
            axes[1, col].set_ylabel('Weighted density')
            axes[1, col].set_title(f'y-marginal ($N_{{tgt}}$={n_sel})')
        ...
        for col in range(len(remaining), n_cols):
            axes[0, col].set_visible(False)
            axes[1, col].set_visible(False)
        plt.suptitle('Weighted spatial marginals at t=6.0 (last 3)',
                     fontsize=14, y=1.01)
        plt.tight_layout()
        plt.savefig(os.path.join(SCRIPT_DIR, "analysis_marginals_2.png"),
                    dpi=150, bbox_inches='tight')
        plt.close()
        print("Saved analysis_marginals_2.png")

    # ==================================================================
    # ANALYSIS PLOT 4: Decay curves for selected N_target
    # ==================================================================
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(18, 5*n_rows))
    axes_flat = axes.ravel()
    for col, n_sel in enumerate(SELECT_N):
        ax = axes_flat[col]
        s = all_analysis[n_sel]
        ...
        ax.set_title(f'Particle decay ($N_{{tgt}}$={n_sel})')
    ...
    # ==================================================================
    # ANALYSIS PLOT 5: Sample trajectories for selected N_target
    # ==================================================================
    ...
    # Target row + one row per N_target → (1+6) rows × 1 col is too tall
    # Use: target in first column, learned in remaining — 2 rows × 3 cols each
    # Plot target once, then 2x3 grid of learned trajectories
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(18, 6*n_rows))
    axes_flat = axes.ravel()

    for i, n_sel in enumerate(SELECT_N):
        ax = axes_flat[i]
        lrn_z_np = all_analysis[n_sel]['all_z']
        lrn_S_np = all_analysis[n_sel]['all_S']
        for idx in traj_idx:
        ...
        ax.set_title(f'Learned Ntgt={n_sel}')
        ax.set_aspect('equal')
    ...
    # ==================================================================
    # ANALYSIS PLOT 6: Zone weight fractions vs N_target
    # ==================================================================
    ...
    # ==================================================================
    # ANALYSIS PLOT 7: Summary metrics vs N_target
    # ==================================================================
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
